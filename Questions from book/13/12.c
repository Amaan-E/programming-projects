/*
Write a program that reverses the words in a words:

Enter a words: you can cage a swallow can't you?
Reversal of words: you can't swallow a cage can you?
...
/*
Modify Programming Project 14 from Chapter 8 so that it stores the words in a two-dimensional char array as it reads the words, with each row of the array storing a single word. Assume that the words contains no more than 30 words and no word is more than 20 characters long. Be sure to store a null character at the end of each word so that it can be treated as a string.
*/

#include <stdio.h>

#define DEBUG 1
//the words contains no more than 30 words and no word is more than 20 characters long
...
/*
	int k = 0;
	for ( i = 0; ( words[ i ] = getchar( ) ) != '\n'; i++, k++ ) {
		words[ i ][ k ] = '\0';
	}	 
	terminatingCharacter = words[ i ][k];
*/
	int k = 0;
	for ( i = 0, k = 0; words[ i ] != '; i++, k++ ) {
		scanf( "%s", words[ i ] );
		words[ i ][ k ] = '\0';
	}
...
/*	int j;
	while ( i != -1 ) {
		...
		if ( words[ i ] == ' ') {
			i++;
			while ( words[ i ] != ' ' && words[ i ] != terminatingCharacter ) {
				putchar( words[ i++ ] );
			}
			putchar( ' ');
		}
...
```c
while (words[i] != ' ' && words[i] != terminatingCharacter && words[i] != '\0') {
    putchar(words[i++]);
}
```
...
1. **Purpose of the Loop**: The loop is intended to continue printing characters from the `words` array as long as the current character (`words[i]`) is **not** a space, **not** the terminating character, and **not** the null terminator (`'\0'`).

2. **Using `&&` (Logical AND)**:
   - The loop will keep executing as long as **all** conditions are true.
   - If any one of the conditions becomes false (i.e., if `words[i]` is a space, the terminating character, or the null terminator), the loop will stop.
...
- For example, if `words[i]` is a letter (which is neither a space nor the terminating character nor the null terminator), the condition would be true, and the loop would continue.
- However, if `words[i]` were a space, the first condition (`words[i] != ' '`) would be false, but the other conditions would still be true, causing the loop to continue indefinitely (or until it accesses out of bounds).
