...
int power( int x, int n ) { 
/*	If n is a power of 2, then x^n can be computed by squaring. For example, x^4 is the square of x^2, so x^4 can be computed using only two multiplications instead of three.
	If n is even, we use the formula x^n = (x^n/2)^2. If n is odd, then x^n = x * x^n-1.
*/
	if ( n == 0 ) {
		return 1;
	} else { 
